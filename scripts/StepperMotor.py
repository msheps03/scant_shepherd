...
class StepperMotor:
    ...
    def setScanRange(self):
        ...
        if len(self.scanPos) == 0:  # if empty, try an if not self.scan_pos:
    ...
    def setStepMode(self):
        os.system('ticcmd --step-mode ' + str(self.stepMode) + ' -d ' + self.ID)
        return
    ...
    def setMaxAccel(self):
        os.system('ticcmd --max-accel ' + str(self.maxAccel) + ' -d ' + self.ID)
        return

    def setMaxSpeed(self): 
        os.system('ticcmd --max-speed ' + str(self.maxVelocity) + ' -d ' + self.ID)
        return
